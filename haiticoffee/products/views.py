...
from haitiApp.models import Product, Product_Image
import json
...
def products(request):
    try:
        if request.method == "GET":
            ...
            productObj = Product.objects.filter(id=currProduct.id).values()
            ...
            relatedImages = Product_Image.objects.filter(product=productToDelete)
...
def manageProductImages(request, product_id):
    try:
        ...
        if request.method == "GET":
            ...
            addImageToProduct(request.FILES['productImage'], currProduct)
        elif request.method == "DELETE":
            # is given img id in request body
